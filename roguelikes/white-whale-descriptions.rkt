...
(define-syntax range-cond 
  (syntax-rules ()
    [(range-cond val) (error "No match for range-cond, ~a" val)]
    [(range-cond ((min0 max0) expr0 ...) ((min max) expr ...) ...)
     (let ((id val))
       (if (and (>= id min0) (<= max0)) (begin expr0 ...)
           (range-cond id ((min max) expr ...) ...)))]))

(description-of player
                (let ((health-percentage 
                       (match player
                         [(hash-table 
                           ('health 
                            (hash-table 
