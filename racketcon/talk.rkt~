...
(require 
 ...
 (only-in utilities/sprite-loading load-image)
...
(slide #:title "What we want to accomplish"
       ...
       (item "Justify this approach by extending our monad to add powerful new features to our system."))
...
(slide #:title "Turtle Observations"
       ...
       (item "Supposing we want to simulate our turtle without side effects.  How?"))
...
(slide #:title "Purely:"
       ...
       (item "...at least a new state.")
       (item "We'll also let turtle functions return a value in addition to a new state.")
       (item "That is, commands or turtle functions return a doublet of values, the first an arbitrary value (for subsequent commands) and the second a new state.")
       ) 

(slide #:title "Useful Operations Look Like:"
...
(slide #:title "Connecting Turtle Functions"
       (item "Insert a value into a stream of turtle functions.")
  (item (code (define (turtle-return item) 
                (lambda (turtle-state)
                  (doublet item turtle-state)))))
  (item "Combine a turtle function with a parameterized turtle function.")
  (item (code (define (turtle-bind turtle-function
                                   turtle-function-producer)
                (lambda (turtle-state)
                  (match (turtle-function turtle-state)
                    [(doublet proper-return-val new-state)
                     (let ((new-turtle-fun (turtle-function-producer proper-return-val)))
                       (new-turtle-fun new-state))]))))))

(slide #:title "At the Monads of Madness"
       ...
       (item "A collection of data types which satisfy certain constraints, like:"
             (subitem "There exist operations `bind` and `return` and types `monadic values,` `monadic-functions` and `values` for which (among other things):")
...
(slide #:title "Yeah, but what is a monad?"
       (item "Ok, ok - a monad is (at least) a way of extending the idea of variable binding. Consider that:")
       (item (code (let* ((x 10)
                          (y (+ x 11)))
                     (+ x y))))
...
(slide #:title "Extending the State Monad"
       ...
       (item "They also should return (somehow) a list of multiple outcomes."))
...
(slide #:title "lots of code: turtles-bind"
       (code (define (turtles-bind turtlesf turtlesf-prod)
  (lambda (state-doublet)
    (match (turtlesf state-doublet)
      [(doublet local-doublets global-state)
       "loop fold the global-state through all the new turtle functions, producing one for each proper return value in local-doublets."
       (let loop ((local-doublets local-doublets)
         ...
         (match local-doublets
           ...
           [(cons local-doublet local-doublets)
            (match local-doublet
              [(doublet proper-return-val local-state)
               (let ((new-turtle-fun (turtlesf-prod proper-return-val)))
                 "Apply the new function, accumulating the local and global parts."
                        ...
                        (code (turtles->pict 
                               (turtles-let* 
                                ((pos (split-set 'pos 
                                                 ...
                                                 (point 225 225)
                                                 (point 75 225)))
                                 (eccentricity (if (= (point-x pos)
                                                      (point-y pos))
                                                   (turtles-return 1.1)
                                                   (turtles-return 0.9)))
                                 (final-pos (spiraling-shape eccentricity 1.1)))
                                (turtles-return final-pos))))))
       (item (mts:turtles->pict 
              (mts:turtles-let* 
               ((pos (mts:split-set 'pos 
                                    ...
                                    (pg:point 225 225)
                                    (pg:point 75 225)))
                (eccentricity (if (= (pg:point-x pos)
                                     (pg:point-y pos))
                                  (mts:turtles-return 1.1)
                                  (mts:turtles-return 0.9)))
                (final-pos (spiraling-shape 40 eccentricity)))
               (mts:turtles-return final-pos)))))

(slide #:title "Conclusions"
       (item "Monadic computations facilitate elegant representations of purely functional code by extending the notion of variable binding.")
       ...
       (item "Check my blog for a screencast version (and other Scheme/Lisp/Programming related content: dorophone.blogspot.com"))

(slide #:title "Thanks/Future:"
       (item "Typed racket for genealized monadic binding expressions?")
       (item "Thanks: Triangle Functional Programmers, Konrad Hinson's (Clojure Monads), Jim Duey (Monad Tutorials in Clojure)."))
